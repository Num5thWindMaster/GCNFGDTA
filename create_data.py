...
if __name__ == '__main__':
    ...
    for dataset in datasets:
        if dataset in ['bindingdb']:  ## bindingdb dataset already split up, skip
            # 设置随机种子
            random_seed = 6
            # 读取数据集
            input_file = 'data/bindingdb/Ki_data_filtered.csv'
            data = pd.read_csv(input_file)
            # 按照 5:1 划分训练集和测试集
            train_data, test_data = train_test_split(data, test_size=1 / 6, random_state=random_seed)
            # 创建保存目录
            output_dir = 'data'
            os.makedirs(output_dir, exist_ok=True)
            # 保存训练集和测试集
            train_file = os.path.join(output_dir, 'bindingdb_train.csv')
            test_file = os.path.join(output_dir, 'bindingdb_test.csv')
    ...
    for smile in compound_iso_smiles:
        ...
        #         print(f'error smiles: {smile}')
        #     if len(g[2]) == 0:
        #         banned_index.append(smile)
        #     else:
        #         smile_graph[smile] = g
        # print(banned_index)
        g = smile_to_graph(smile)
